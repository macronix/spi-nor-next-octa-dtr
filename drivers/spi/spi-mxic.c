...
static int mxic_spi_clk_setup(struct spi_device *spi)
{
	struct mxic_spi *mxic = spi_master_get_devdata(spi->master);

	if (mxic->cur_speed_hz != spi->max_speed_hz)
...
static void mxic_spi_set_hc_cfg(struct spi_device *spi, u32 flags)
{
	...
	if (spi->mode & (SPI_RX_OCTO | SPI_TX_OCTO))
		nio = 8;
...
static void mxic_spi_set_input_delay_dqs(struct mxic_spi *mxic, u8 idly_code)
{
	...
	/* set Input delay code when DQS_EN = 1 */
	writel(IDLY_CODE_VAL(0, idly_code) |
	       IDLY_CODE_VAL(1, idly_code) |
	       IDLY_CODE_VAL(2, idly_code) |
	       IDLY_CODE_VAL(3, idly_code),
	       mxic->regs + IDLY_CODE(4));
	writel(IDLY_CODE_VAL(4, idly_code) |
	       IDLY_CODE_VAL(5, idly_code) |
	       IDLY_CODE_VAL(6, idly_code) |
	       IDLY_CODE_VAL(7, idly_code),
	       mxic->regs + IDLY_CODE(5));
}
...
static int __maybe_unused mxic_spi_resume(struct device *dev)
{
	struct spi_master *master = dev_get_drvdata(dev);
	struct mxic_spi *mxic = spi_master_get_devdata(master);
	int ret;
